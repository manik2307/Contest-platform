...
public class UserService implements IUserService {
    ...
    public User create(String name) {
        User user = new User(name, 0); // Assuming initial score is 0
    ...
    if (contest.getContestStatus() != ContestStatus.NOT_STARTED) {
        throw new InvalidOperationException("Contest is not available for withdrawal");
    }

    if (!user.checkIfContestExists(contest)) {
        throw new InvalidOperationException("User not registered for the contest");
    }
